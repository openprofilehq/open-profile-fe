...
export default function DefaultPreview({
  ...
  const highlightedProject = projectsToRender.find(p => p.highlighted === true || String(p.highlighted) === "true");
  const remainingProjects = projectsToRender.filter(p => p.id !== highlightedProject?.id);

  const rawPhotoUrl = profile?.photoUrl;
  const profileImageUrl = rawPhotoUrl ? (rawPhotoUrl.startsWith("/profile-preview/") ? rawPhotoUrl : getImageUrl(rawPhotoUrl)) : null;
              ...
              return remainingProjects.length > 0 ? (
                <div className={`grid gap-6 p-6 ${
                  ...
                  {remainingProjects.map((project, index) => {
                    ...
                    const rawImageSrc = (project as { imageSrc?: string }).imageSrc;
                    const displayImg = rawImageSrc 
                      ? (rawImageSrc.startsWith("/profile-preview/") ? rawImageSrc : getImageUrl(rawImageSrc))
                      : null;
                      
                    const card = (
                      <div className={`group flex ${projectsSection.layout === "1" ? "flex-col sm:flex-row sm:items-center" : "flex-col"} gap-4`}>
                        <div className={`border-border bg-secondary-bg relative shrink-0 overflow-hidden rounded-[8px] border shadow-sm ${
                          projectsSection.layout === "1" ? "w-full sm:w-[280px] aspect-video" : "w-full aspect-video"
                        }`}>
                          {displayImg ? (
                            <Image
                              src={displayImg}
                            ...
                            <div className="flex h-full w-full items-center justify-center bg-neutral-200 text-sm text-tertiary-text">
                              No image
                            </div>
                          )}
                        </div>
                        <div className="flex flex-col gap-1 min-w-0 flex-1">
                          <h3 className="font-bold text-primary-text break-all">{project.title}</h3>
                          {project.description && (
                            <p className="line-clamp-2 text-sm text-secondary-text break-all">
                              {project.description}
                            </p>
                          )}
                          {hasUrl && projectsSection.layout !== "1" && (
                            <span className="mt-1 flex items-center gap-1 text-sm font-semibold text-brand-hover-bg group-hover:underline">
                              {(project as { buttonText?: string }).buttonText || "View project"} <ExternalLink size={14} />
                            </span>
                          )}
                        </div>
                        ...
                        {projectsSection.layout === "1" && (
                          <div className="mt-4 sm:mt-0 sm:ml-6 shrink-0">
                            <span className="flex items-center gap-1 text-sm font-bold text-brand-hover-bg hover:underline">
                              {(project as { buttonText?: string }).buttonText || "View project"}
                    ...
                    return hasUrl ? (
                      ...
                      <div key={project.id || index} className="block no-underline">
                        {card}
                      </div>
                    );
                  })}
